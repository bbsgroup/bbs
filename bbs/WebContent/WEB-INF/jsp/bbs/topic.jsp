							...
							<td class="postauthor" rowspan="3"><cite><a
								...
								<div class="avatar">
									<a href="javascript:void(0);" target="_blank" title="系统管理员">
										<c:if test="${topic.author.userInfo.showHead}">
											<IMG src="${topic.author.userInfo.heanImage }" border=0
												onload="resizeImage(this, 150);">
										</c:if> <c:if test="!${topic.author.userInfo.showHead}">
											<IMG
										...
										<div class="box attachlist">
											...
											<c:forEach items="${topic.attachments }" var="attach">
												<dl class="p_attachlist">
													<dt>
														<img
															src="${pageContext.request.contextPath}/media/images/i_attach.gif"
															border="0" class="absmiddle">&nbsp;&nbsp; <a
															href="${pageContext.request.contextPath}/downloadAttachment?id=${attach.id}"
															class="bold">${attach.filename }</a>&nbsp;&nbsp;<em
															class="num">(<fmt:formatNumber
																value=" ${attach.size/1024}" pattern="#,#00.0#" /> KB)
														</em>
													</dt>
													<dd>
														<p>
								...
								<div class="postactions">
									<p>
										...
										<c:if test="${isModerator }">
											<a
												href="${pageContext.request.contextPath}/deleteTopicContent?id=${topic.id}"
						...
						<tbody>
							<tr>
								<td class="postauthor" rowspan="3"><cite><a
									...
									<div class="avatar">
										<a href="javascript:void(0);" target="_blank"><c:if
												test="${reply.author.userInfo.showHead}">
												<IMG src="${reply.author.userInfo.heanImage }" border=0
													onload="resizeImage(this, 150);">
											</c:if> <c:if test="!${reply.author.userInfo.showHead}">
												<IMG
													src="${pageContext.request.contextPath}/media/images/avatar/unknown.gif"
													border=0 onload="resizeImage(this, 150);">
											</c:if></a>
