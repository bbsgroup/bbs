...
var is_firefox = (navigator.product == 'Gecko' && userAgent.indexOf('firefox') > 0);
if (is_firefox) {
...
function showSlide(url, width, height){
	...
	document.write('<embed src="' + url + '" wmode="opaque" quality="high" menu="false" ');
	document.write('FlashVars="config='+config+'&bcastr_flie='+files+'&bcastr_link='+links+'&bcastr_title='+texts+'" ');
	document.write('width="'+ width + '" height="'+ height + '" ');
	document.write('type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer"/>');
	document.write('</object>');
}
...
function doSearch() {
	...
	var x = new Ajax();
   	x.get('ajax?act=statkeys&keys=' + encodeURIComponent(trim($('frmsearch').q.value)), null);
	frmsearch.submit();
}

function searchImages(url) {
	window.open("http://images.google.com.hk/images?hl=zh-CN&as_sitesearch=" + url);
}
